/* =====================================================================
   FOR ADITIYA — a birthday adventure by Jyoti
...
(() => {
  ...
  function reset(){used=goals=saved=0;locked=false; score.textContent="0";shots.textContent="0";saves.textContent="0";attempts.textContent="3 SHOTS";commentary.textContent="Aditiya steps up..."; keeper.className="keeper";}
  function shoot(zone){
...
function openThemeSecret(kind){
  ...
  if(!data)return; box.className="secret-scene "+data.className; box.innerHTML=`<div class="secret-icon">${data.icon}</div><h2>${data.title}</h2><p>${data.msg}</p><strong>Happy Birthday, Aditiya ❤️</strong>`; overlay.hidden=false; overlay.classList.add("show"); AudioManager.play(kind==="batman"?"bat":kind==="spider"?"webSwing":"kick");
...
(() => {
  ...
  onEnter(scenes.FINAL, () => {
    ...
    if (EasterEggs.allFound()) {
      ...
      title.textContent = "⚽ 🦇 🕷️ ADITIYA — THE LEGEND";
      sub.innerHTML = "FRIEND · BROTHER · CHAOS PARTNER · MEMORY MAKER · LEGEND<br><b>HAPPY BIRTHDAY, ADITIYA ❤️</b>";
    }
  });
})();
